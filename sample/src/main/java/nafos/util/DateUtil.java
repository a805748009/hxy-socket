...
public class DateUtil {
    ...
    public static String formatKMGB(long data) {
        ...
        if (data < 1024) {
            ...
            // 因此，把此数乘以100之后再取余
            data = data * 100;
            return String.valueOf((data / 100)) + "." + String.valueOf((data % 100)) + "MB";
        } else {
            // 否则如果要以GB为单位的，先除于1024再作同样的处理
            data = data * 100 / 1024;
            return String.valueOf((data / 100)) + "." + String.valueOf((data % 100)) + "GB";
        }
    }
    ...
    public static long getRank(int order, int level, int total) {
        long rank = Long.parseLong(String.valueOf(Math.pow(10, (total - level + 2))));
        return order * rank;
    }
    ...
    public static long getRank2(long prank, long order, int level, int total) {
        return prank + Long.parseLong(String.valueOf(Math.pow(10, (total - level + 2)))) * order;
    }
    ...
    public static String formatDate(Date date, String formatStyle) {
        if (date == null || formatStyle == null && formatStyle.trim().equals("")) return null;
