...
public class BSRedisUtil {
	
	/**
	 * 这里在login服务器中，主要做session的缓存。
	 */
	private static final Logger logger = LoggerFactory.getLogger(BSRedisUtil.class);
	//本机redis连接池
	...
	public static boolean set(String key, Object value) {
		Jedis jedis = null;
		try {
			String objectstr = SerializationUtil.serializeToString(value);
			jedis = getJedis();
			if (jedis != null) {
				jedis.set(key, objectstr);
			}
			return true;
		} catch (Exception e) {
			logger.error(e.toString());
	...
	public static  boolean setMapExpire(String key,Map<String,String> map, int seconds){
		Jedis jedis = null;
		try {
			jedis = getJedis();
			if (jedis != null) {
	...
	public static boolean setMapKey(String key,String mapKey,String value){
		Jedis jedis = null;
		try {
			jedis = getJedis();
			if (jedis != null) {
		...
		try {
			...
			if (jedis != null) {
				...
				map.keySet().forEach(str ->{
					newMap.put(str, SerializationUtil.deserializeFromString(map.get(str),clazz));
				});
				return newMap;
			} else {
				return null;
	...
	public static <T> T getMapKeyEntity(String key,String mapKey,
		...
		try {
			...
			if(ObjectUtil.isNull(value))
				return null;
			return SerializationUtil.deserializeFromString(value,entityClass);
			}else{return null;}
		} catch (Exception e) {
			logger.error(e.toString());
			return null;
	...
	public static String getMapKey(String key,String mapKey){
		ShardedJedis jedis = null;
		try {
			jedis = getShardedJedis();
			if(jedis != null){
	...
	public static boolean hincrBy(String key,String field, int value){
		Jedis jedis = null;
		try {
			jedis = getJedis();
